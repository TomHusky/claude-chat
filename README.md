...
- ⏱ **还原点**：每轮对话前自动建点。一键还原会**同时**回滚工作区文件、**并真正回退对话**（用 CLI 官方的 `resumeSessionAt` 从该点续接，与 Claude Code 自己的 /rewind 同一机制，Claude 会真的"忘记"之后的轮次，`.jsonl` 一行不动）；被回退的那条提问连同图片附件自动带回输入框，改完即可重发。截断前有安全校验，还原点与对话对不上时中止，绝不误删上下文
...
- **还原点的真实回退**：每轮发送前记录该 `.jsonl` 的行数；还原时先结束当前进程，取该行之前最后一条链记录作为回退点，下一次拉起进程用 `--resume` + `resumeSessionAt` 从该点续接（Claude Code 自己的 /rewind 就是这么做的）——新消息从回退点接着写，之后的记录成为被放弃的分支，文件本身一行不动。此前按行截断文件，垂死的 CLI 会在截断后再刷出几条记录，一旦其中有链记录，`parentUuid` 链就断了：官方插件只剩尾巴、`--resume` 丢上下文——这就是改用官方机制的原因。历史渲染同样沿 `parentUuid` 链读取，被放弃的分支不显示。
