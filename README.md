...
- ⏱ **还原点**：每轮对话前自动建点；由官方插件 / 其它窗口发出的轮次没有文件快照，也会从对话记录合成还原点（只回退对话、不回滚文件，悬停有说明），所以每一轮都能「还原到此处」。一键还原会**同时**回滚工作区文件、**并真正回退对话**（用 SDK 官方的 `forkSession` 从该点派生出干净的新会话并静默替换当前会话，Claude 会真的"忘记"之后的轮次，官方插件看到的也立即一致）；被回退的那条提问连同图片附件自动带回输入框，改完即可重发。截断前有安全校验，还原点与对话对不上时中止，绝不误删上下文
...
- **还原点的真实回退**：每轮发送前记录该 `.jsonl` 的行数；还原时先结束当前进程，取该行之前最后一条链记录，用 SDK 的 `forkSession(upToMessageId)` 派生出一个只含此前对话的新会话（沿 `parentUuid` 链复制、UUID 重映射、跨压缩边界保留全部历史），当前标签页静默切到新会话、旧会话删除，还原点按提问文本重新对齐到新文件——用户看到的还是同一段对话，而官方插件与 `--resume` 立即一致。此前按行截断文件，垂死的 CLI 会在截断后再刷出几条记录，一旦其中有链记录，`parentUuid` 链就断了：官方插件只剩尾巴、`--resume` 丢上下文——这就是改用官方机制的原因。历史回放按文件顺序读取，只跳过从主链分叉出去的被放弃分支（回退/编辑重发留下的旧尾巴）；与主链不相连的孤儿记录照常显示，老会话的内容与还原点不受影响。
...
- **文件快照只在「从插件输入框发出消息」时建立**：其它入口（官方插件、其它窗口）发出的轮次只有合成还原点，能回退对话但不能回滚文件。
